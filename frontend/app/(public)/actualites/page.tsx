...
function NewsCard({ item }: { item: NewsItem }) {
  const newsType = item.type || "annonce"
  const typeInfo = typeLabels[newsType]
  const Icon = typeIcons[newsType] || Newspaper
      ...
      <CardContent className="flex h-full flex-col p-6">
        ...
        <div className="mt-3 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{item.content.slice(0, 120)}...</p>
          </div>
        </div>
...
export default function ActualitesPage() {
  ...
  useEffect(() => {
    const loadNews = async () => {
      try {
        const payload = await apiFetch<NewsPaginatedResponse>("/news")
  ...
  const annonces = allNews.filter((n) => (n.type || "annonce") === "annonce")
  const economie = allNews.filter((n) => n.type === "economie")
  const evenements = allNews.filter((n) => n.type === "evenement")
